...
const SV = (() => {
  ...
  // SVG <text> 不會經過 MathJax；把教材字串中的常見 LaTeX 標記轉成可直接顯示的文字。
  const subscript = { '0': '₀', '1': '₁', '2': '₂', '3': '₃', '4': '₄', '5': '₅', '6': '₆', '7': '₇', '8': '₈', '9': '₉', '+': '₊', '-': '₋', '=': '₌', '(': '₍', ')': '₎' };
  const superscript = { '0': '⁰', '1': '¹', '2': '²', '3': '³', '4': '⁴', '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹', '+': '⁺', '-': '⁻', '=': '⁼', '(': '⁽', ')': '⁾', 'n': 'ⁿ' };
  const unicodeChars = (value, map) => [...value].map(ch => map[ch] || ch).join('');
  const text = (value) => {
    let out = String(value)
      ...
      .replace(/_\{([^{}]*)\}/g, (_, value) => unicodeChars(value, subscript))
      .replace(/\^\{([^{}]*)\}/g, (_, value) => unicodeChars(value, superscript))
      .replace(/_([0-9A-Za-z])/g, (_, value) => unicodeChars(value, subscript))
      .replace(/\^([0-9A-Za-z+\-=])/g, (_, value) => unicodeChars(value, superscript))
      .replace(/[{}]/g, '')
      .replace(/\\/g, '');
  ...
  return { pt, angleOf, arcPoints, angle, rightAngle, ticks, dot, vlabel, seg, poly, arrowDefs, plane, RAD, fbox, stepper, text };
})();
